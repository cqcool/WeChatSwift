...
@interface BWLogExportManager ()<NSStreamDelegate>
{
    NSString *_defaultLogFilePath;
    //    FILE *_logFile;
}
@property (nonatomic, strong)  NSInputStream * inputStream;
/// 文件 读取字节流数据、用来计算 offset
...
/// 完整日志压缩包路径
@property (nonatomic, copy) NSString *archiveLogPath;
...
- (void)startLogRecord {
    _logging = YES;
    [self startLogRecordWithXcodeEnable:YES];
}
...
- (void)exportLogFileInViewController:(UIViewController *)viewController {
    ...
    NSURL *fileURL = nil;
    // 判断压缩文件是否存在
    if ([BWLogExportManager existsFileAtPath:self.archiveLogPath]) {
        fileURL = [NSURL fileURLWithPath:self.archiveLogPath];
    } else {
...
- (BOOL)compressFiles {
    // NSLog.txt 文件路径
    NSString *file1Path = _logFilePath;
    // 数据库文件路径
    NSString *documentsDirectory = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject];
    NSString *file2Path  = [DBManager dbPath];
    NSString *zipName = [NSString stringWithFormat:@"app_%@.zip",[WXDevice appVersion]];
    NSString *zipFilePath = [_logFolderPath stringByAppendingPathComponent:zipName];
    _archiveLogPath = zipFilePath;
    BOOL success =  [SSZipArchive createZipFileAtPath:zipFilePath withFilesAtPaths:@[file2Path]];
    if (success) {
        NSLog(@"压缩成功");
    } else {
        NSLog(@"压缩失败");
